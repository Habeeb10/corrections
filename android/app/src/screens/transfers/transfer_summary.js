...
class TransferSummary extends Component {
    ...
    handleSubmit = (pin) => {
        ...
        this.setState({
            ...
            const {BankVerificationNumber,DestinationInstitutionCode,SessionID,ChannelCode,KYCLevel, amount, account_number, account_name, beneficiary_id, save_beneficiary, bank, narration } = this.state;
            let payload;
            if (this.props.transfers.account_type==account_type_data.ACCOUNT_TYPE.TOUCH_GOLD) {
                payload={
             ...
             payload = {
                ...
                OriginatorAccountName: this.props.user.user_data.firstName+" "+this.props.user.user_data.lastName,
            ...
            .then(() => {
                const apiCall=this.props.transfers.account_type==account_type_data.ACCOUNT_TYPE.OTHERS ?Network.doTransfer:Network.doTransferIntra;
                apiCall(payload).then((result) => {
                    this.setState({
                        ...
                        this.props.navigation.navigate('Success', {
                            ...
                            event_data: { transaction_id: result.referenceNumber, amount },
                            success_message: result.resp.message,
                            transaction_data: result.intraDepositResp||result.transferData,
                        ...
                        this.props.navigation.navigate('Error', {
                            ...
                            error_message: error.message ?? "Transfer is not successfull,Try again."
                        });
                    }
                    });
                }).catch((error) => {
    ...
    render() {
        const { amount, account_number, account_name, bank, narration, fees } = this.state;
        const total_fees = 0;
                        ...
                        <View style={SharedStyle.section}>
                            <View style={SharedStyle.row}>
                                <View style={SharedStyle.halfColumn}>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.BENEFICIARY_NAME_LABEL}</Text>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value]}>{account_name}</Text>
                                </View>
                                <View style={SharedStyle.halfColumn}>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.BANK_LABEL}</Text>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.right, {color: Colors.DARK_GREY}]}>{bank?.label ?? bank}</Text>
                                </View>
                            </View>
                            <View style={SharedStyle.row}>
                                <View style={SharedStyle.halfColumn}>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.ACCOUNT_NUMBER_LABEL}</Text>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value]}>{account_number}</Text>
                                </View>
                                <View style={SharedStyle.halfColumn}>
                            ...
                            <View style={SharedStyle.row}>
                                <View style={SharedStyle.fullColumn}>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.NARRATION}</Text>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value]}>{narration || '- - - '}</Text>
                                </View>
                            </View>
                            <TouchItem
...
export default connect(mapStateToProps, mapDispatchToProps)(withNavigationFocus(TransferSummary));