...
class SavingsWithdrawal extends Component {
    ...
    validateWithdrawal = () => {
        ...
        if (!savings.is_matured) {
            this.setState({
                ...
                Network.getWithdrawalWarning(savings.id, amount).then((result) => {
                    this.setState({
                        processing: false,
                    }, () => {
                        if (result?.resp && result?.resp.code== ResponseCodes.SUCCESS_CODE) {
                            this.setState({
        ...
        this.setState({
            ...
            Network.validatePIN(pin)
                .then(() => {
                    ...
                    Network.withdrawSavings(payload).then((result) => {
                        this.setState({
                            processing: false,
                            auth_modal_visible: false
                        }, () => {
                            ...
                            this.props.navigation.navigate('Success', {
                                ...
                                success_message: result.data?.message || Dictionary.SAVINGS_WITHDRAWN
                            });
    ...
    render() {
        const { savings, processing, penal_warning, penal_warning_visible, auth_modal_visible } = this.state;
        return (
            <View style={SharedStyle.mainContainer}>
                <MainHeader leftIcon="arrow-left" onPressLeftIcon={this.handleBackButton} title={Dictionary.SAVINGS_WITHDRAWAL} />
                    ...
                    <View style={FormStyle.formContainer}>
                        <View style={FormStyle.formItem}>
                            ...
                            <Text style={SharedStyle.balanceLabel}>
                                {Dictionary.BALANCE} <Text style={SharedStyle.balanceValue}>₦{Util.formatAmount(Number(savings.balance))}</Text>
                            </Text>
                        </View>
                    </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(withNavigationFocus(SavingsWithdrawal));