...
class SavingsSummary extends Component {
            ...
            Network.validatePIN(pin)
                .then(() => {
                    ...
                    let payload = {
                        ...
                        schedule: {
                            scheduleStartDate: moment(start_date, 'DD/MM/YYYY').format('YYYY-MM-DD'),
                         //   scheduleEndDate: "2022-09-17",
                        scheduleEndDate: moment(end_date, 'MM/DD/YYYY').format('YYYY-MM-DD'),
                        },
                    ...
                    Network.createSavingsPlan(payload).then((result) => {
                        this.setState({
                            processing: false,
                            auth_screen_visible: false
                        }, () => {
                            ...
                            this.props.navigation.navigate('Success', {
                                ...
                                event_data: { amount },
                                success_message: result.message||`Hurray! We have created your ${savings_product.name} plan. You can now start saving and earning interest.`
                            });
    ...
    render() {
        ...
        let matures_on = moment(start_date, "DD/MM/YYYY").add(duration, preferred_offer.tenor_period).format('D MMM, YYYY');
        return (
            <ImageBackground
                    ...
                    <View style={[SharedStyle.section, styles.section]}>
                        <View style={SharedStyle.row}>
                            <View style={SharedStyle.halfColumn}>
                                ...
                                <Text numberOfLines={2} style={[SharedStyle.normalText, SharedStyle.value, { textTransform: 'none' }]}>{plan_name}</Text>
                            </View>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.PLAN_TYPE}</Text>
                                <Text numberOfLines={2} style={[SharedStyle.normalText, SharedStyle.value, SharedStyle.right, { textTransform: 'none' }]}>{savings_product.name}</Text>
                            </View>
                        </View>
                        <View style={SharedStyle.row}>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.SAVINGS}</Text>
                                <Text numberOfLines={2} style={[SharedStyle.normalText, SharedStyle.value]}>₦{Util.formatAmount(amount)}</Text>
                            </View>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.FREQUENCY}</Text>
                                <Text numberOfLines={2} style={[SharedStyle.normalText, SharedStyle.value, SharedStyle.right]}>{saving_frequency ? saving_frequency.name : '- - -'}</Text>
                            </View>
                        </View>
                        <View style={SharedStyle.row}>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.SAVINGS_TARGET}</Text>
                                <Text numberOfLines={2} style={[SharedStyle.normalText, SharedStyle.value]}>₦{Util.formatAmount(target)}</Text>
                            </View>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.SAVINGS_TYPE}</Text>
                                <Text numberOfLines={2} style={[SharedStyle.normalText, SharedStyle.value, SharedStyle.right]}>{collection_mode.name}</Text>
                            </View>
                        </View>
                        <View style={SharedStyle.row}>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.START_BY}</Text>
                                <Text numberOfLines={2} style={[SharedStyle.normalText, SharedStyle.value]}>{starts_on}</Text>
                            </View>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.WITHDRAW_DATE}</Text>
                                <Text numberOfLines={2} style={[SharedStyle.normalText, SharedStyle.value, SharedStyle.right]}>{matures_on}</Text>
                            </View>
                        </View>
                    </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(withNavigationFocus(SavingsSummary));