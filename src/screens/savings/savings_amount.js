...
import { BackHandler, StyleSheet, Text, View, ScrollView, KeyboardAvoidingView, FlatList } from 'react-native';
...
import { updateSavingsApplicationData, getSavingsCollectionModes } from '_actions/savings_actions';
import { showToast } from '_actions/toast_actions';

import { Dictionary, Util } from '_utils';
import { Colors, Mixins, Typography, SharedStyle, FormStyle } from '_styles';
import { SubHeader, FloatingLabelInput, InfoPanel, TouchItem, ScrollView as _ScrollView } from '_atoms';
...
class SavingsAmount extends Component {
    constructor(props) {
        ...
        const { savings_product, preferred_offer } = this.props.savings.savings_application;

        let amount_range = Dictionary.AMOUNT_RANGE
            .replace("{{min_amount}}", Util.formatAmount(preferred_offer.minimumAmount))
            .replace("{{max_amount}}", Util.formatAmount(preferred_offer.maximumAmount));
    ...
    validFields = () => {
        const { savings_product, preferred_offer, frequency, amount, start_date } = this.state;
        let is_valid = true;
        ...
        if (!Util.isValidAmount(amount) || +amount < preferred_offer.min_amount || +amount > preferred_offer.max_amount) {
    ...
    updateBreakdown = () => {
        ...
        if (savings_product.is_fixed) {
            ...
            for (let i = 0; i < mod; i++) {
                let periodic_interest = 0;
                if (i > 0) {
                    periodic_interest = this.proRatedInterest(savings_balance, interest_rate, duration_type);
                    interest_earned += periodic_interest;  
                }
    ...
    proRatedInterest = (amount_saved, interest_rate, duration_type) => {
        let toatl_period = duration_type === "days" ? 365 : duration_type === "months" ? 12 : 52
...
//
        this.setState({ processing: true }, () => {
            const { savings_product, preferred_offer, amount, frequency, duration, start_date ,breakdown} = this.state;
            let formatted_start_date = moment(start_date, 'DD/MM/YYYY').format('MM/DD/YYYY');
            let end_date = moment(start_date, 'DD/MM/YYYY').add(duration, preferred_offer.tenor_period).format('MM/DD/YYYY');
            ...
            // if (!savings_product.is_fixed) {
                this.props.updateSavingsApplicationData({
                    ...
                    amount,
                    breakdown,
                    target: breakdown.maturity_value - breakdown.withholding_tax,
                    duration,
                    start_date,
                    end_date,
    ...
    handleSavePlanName = () => {
        ...
        let collection_mode = collection_modes.find(mode => mode.slug === (auto_collection_mode ? 'automated' : 'manual'));
    ...
    renderSchedules = ({ item }) => {
        return <View style={SharedStyle.row}>
    ...
    render() {
        ...
        let { savings_product, preferred_offer, amount, duration, frequency, start_date, breakdown } = this.state;
        ...
        return (
            <View style={SharedStyle.mainContainer}>
                <MainHeader leftIcon="arrow-left" onPressLeftIcon={this.handleBackButton} title={Dictionary.NEW_SAVINGS} />
                <_ScrollView {...this.props} hasButtomButtons={true}>
                    <SubHeader text={Dictionary.HOW_TO_SAVE} />
                    <View style={[FormStyle.formContainer, show_penal ? styles.formContainer : {}]}>
                        <View style={!!breakdown ? styles.palette : {}}>
                            {!savings_product.is_fixed && (
                                <View>
                                    <View style={FormStyle.formItem}>
                                        <Text style={styles.duration}>{Dictionary.HOW_OFTEN_TO_SAVE}</Text>
                                    </View>
                                    <View style={FormStyle.formItem}>
                                        <ScrollView
                                            ...
                                            {saving_frequencies.map((frequency, index) => {
                                                return <TouchItem
                                                    ...
                                                    style={[
                                                        ...
                                                        this.state.frequency && this.state.frequency.id === frequency.id ? styles.selectedFrequency : {},
                                                        index === saving_frequencies.length - 1 ? { marginRight: Mixins.scaleSize(16) } : {}]}
                                                    onPress={() => this.handleFrequencyChange(frequency)}>
                                                    <View>
                                                        {(!this.state.frequency || this.state.frequency && this.state.frequency.id != frequency.id) && (
                                                            <View style={[styles.blank, styles.icon]}></View>
                                                        )}
                                                        {!!this.state.frequency && this.state.frequency.id === frequency.id && (
                                                            <Icon.Ionicons style={styles.icon} name={'ios-checkmark-circle'} size={Mixins.scaleFont(22)} color={Colors.SUCCESS} />
                                                        )}
                                                    </View>
                                                    <View>
                                        ...
                                        <Text style={FormStyle.formError}>{this.state.frequency_error}</Text>
                                    </View>
                                </View>
                            )}
                            <View style={FormStyle.formItem}>
                            ...
                            <View style={FormStyle.formItem}>
                                ...
                                />
                                <Text style={styles.amountRange}>{this.state.amount_range}</Text>
                                <Text style={FormStyle.formError}>{this.state.amount_error}</Text>
                            </View>
                            <View style={FormStyle.formItem}>
                                <Text style={FormStyle.sectionLabel}>{Dictionary.HOW_LONG_TO_SAVE}</Text>
                            </View>
                            <View style={FormStyle.formItem}>
                                <Text numberOfLines={1} style={styles.sliderValue}>{duration} {preferred_offer.tenor_period}</Text>
                                <Slider
                                    style={styles.slider}
                                     step={1}
                            ...
                            <View style={FormStyle.formItem}>
                                <Text style={styles.duration}>{Dictionary.WHEN_TO_START_SAVING}</Text>
                            </View>
                            <View style={FormStyle.formItem}>
                                <TouchItem
                                ...
                                <Text style={FormStyle.formError}>{this.state.start_date_error}</Text>
                            </View>
                        </View>
                        {!!breakdown && (
                            <View>
                                <View style={FormStyle.formItem}>
                                    <Text style={styles.breakdownHeader}>{Dictionary.BREAKDOWN}</Text>
                                </View>
                                <View style={styles.breakdown}>
                                    <View style={SharedStyle.row}>
                                        <View style={SharedStyle.halfColumn}>
                                            <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.AMOUNT_SAVED}</Text>
                                            <Text numberOfLines={2} style={[styles.value]}>₦{Util.formatAmount(breakdown.total_contribution)}</Text>
                                        </View>
                                        <View style={SharedStyle.halfColumn}>
                                            <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{`Accrued Interest`}</Text>
                                            <Text numberOfLines={2} style={[styles.value, SharedStyle.right]}>₦{Util.formatAmount(breakdown.interest_earned)}</Text>
                                        </View>
                                    </View>
                                    <View style={SharedStyle.row}>
                                        <View style={SharedStyle.halfColumn}>
                                            ...
                                            <Text numberOfLines={2} style={[styles.value]}>
                                                {`${Dictionary.FROM} ${moment(start_date, 'DD/MM/YYYY').format('D MMM, YYYY')} ${Dictionary.TO} ${moment(breakdown.maturity_date, 'yyyy-MM-DD').format('D MMM, YYYY')}`}
                                            </Text>
                                        </View>
                                        <View style={SharedStyle.halfColumn}>
                                            <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.FREQUENCY}</Text>
                                            <Text numberOfLines={2} style={[styles.value, SharedStyle.right]}>
                                                ₦{Util.formatAmount(amount)} {frequency.name?.toLowerCase()} {Dictionary.FOR} {duration} {preferred_offer.tenor_period}
                                            </Text>
                                        </View>
                                    </View>
                                    <View style={SharedStyle.row}>
                                        <View style={SharedStyle.halfColumn}>
                                            <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.WITHOLDING_TAX}</Text>
                                            <Text numberOfLines={2} style={[styles.value]}>
                                               {breakdown.withholding_tax_rate}{Dictionary.PERCENT_OF_INTEREST}
                                            </Text>
                                        </View>
                                        <View style={SharedStyle.halfColumn}>
                                            <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.MATURITY_AMOUNT}</Text>
                                            <Text numberOfLines={2} style={[styles.value, SharedStyle.right]}>₦{Util.formatAmount(breakdown.maturity_value)}</Text>
                                        </View>
                                    </View>
                                </View>
                                {breakdown.schedules.length > 0 && (
                                    <View>
                                        <View style={FormStyle.formItem}>
                                            <Text style={styles.breakdownHeader}>{Dictionary.SCHEDULES}</Text>
                                        </View>
                                        <View style={styles.breakdown}>
                                            <View style={SharedStyle.row}>
                                                <View style={SharedStyle.triColumn}>
                                                ...
                                                <View style={SharedStyle.triColumn}>
                                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.SAVED}</Text>
                                                </View>
                                                <View style={SharedStyle.triColumn}>
                ...
                <Modal
                    ...
                    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? "position" : ""}>
                        <View style={[SharedStyle.modalContent, { height: savings_product.is_fixed ? Mixins.scaleSize(200) : Mixins.scaleSize(300) }]}>
                            <View style={SharedStyle.modalSlider} />
                            <View style={SharedStyle.modalPanel}>
                                <View style={styles.modalMiddle}>
                                    <View style={{ flex: 1 }}>
                                        <Text style={styles.modalLabel}>{Dictionary.NAME_YOUR_SAVINGS}</Text>
                                        <FloatingLabelInput
                                        ...
                                        />
                                        <Text style={FormStyle.formError}>{this.state.plan_name_error}</Text>
                                        {!savings_product.is_fixed && (
                                            <View>
                                                <View style={styles.switchHead}>
                                                    <Text style={styles.modalLabel}>{Dictionary.AUTOMATE_YOUR_SAVINGS}</Text>
                                                    <SwitchToggle
...
export default connect(mapStateToProps, mapDispatchToProps)(withNavigationFocus(SavingsAmount));