...
import { getUserSavings, updateSavingsArchivedStatus } from '_actions/savings_actions';
...
class SavingsDetail extends Component {
    ...
    toggleArchiveSavings = () => {
        ...
        this.setState({
            ...
            Network.archiveSavingsPlan(payload).then(() => {
                this.setState({
                    archiving: false
                }, () => {
                    this.props.updateSavingsArchivedStatus(savings.id, !savings.archived)
    ...
    render() {
        ...
        const disable_actions = (savings.is_matured !== 1 && savings.locked) || savings.archived;
            ...
            <View style={SharedStyle.mainContainer}>
                ...
                <ScrollView {...this.props}>
                    <View style={styles.topContainer}>
                        <TouchItem style={styles.historyButton}
                            onPress={() => this.props.navigation.navigate('SavingsTransactions', { savings })}>
                            <Text style={styles.historyButtonText}>{Dictionary.SAVINGS_HISTORY_BTN}</Text>
                        </TouchItem>
                        <SavingsSummary
                        ...
                        <View style={styles.actionPanel}>
                            <TouchItem
                                ...
                                onPress={()  => this.props.navigation.navigate('SavingsTopUp', { savings })}
                            ...
                            <TouchItem
                                style={[styles.actionButton, disable_actions ? styles.disabled : {}]}
                                onPress={() => this.props.navigation.navigate('SavingsWithdrawal', { savings })}
                            ...
                            <TouchItem
                                style={[
                                    styles.actionButton,
                                    (!savings.data_sources?.pecunia || !savings.is_matured || disable_actions) ? styles.disabled : {}]}
                                onPress={() => this.props.navigation.navigate('RolloverSavings', { savings })}
                                disabled={!savings.data_sources?.pecunia || !savings.is_matured || disable_actions}
                                >
                                <Image
                                    style={styles.actionButtonIcon}
                    ...
                    <View style={styles.bottomContainer}>
                        <View style={styles.row}>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.NAME}</Text>
                        ...
                        <View style={styles.row}>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.MATURITY_INTEREST}</Text>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value]}>₦{Util.formatAmount(savings.interest_due_at_maturity)}</Text>
                            </View>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.ACCRUED_INTEREST}</Text>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value, SharedStyle.right]}>₦{Util.formatAmount(savings.accrued_interest)}</Text>
                            </View>
                        </View>
                        <View style={styles.row}>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary._WITHOLDING_TAX}</Text>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value]}>{savings.witholding_tax}</Text>
                            </View>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.MATURITY_AMOUNT}</Text>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value, SharedStyle.right]}>₦{Util.formatAmount(savings.amount_at_maturity)}</Text>
                            </View>
                        </View>
                        <View style={styles.row}>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.START_DATE}</Text>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value]}>
                                    {isMoment(moment(savings.start_date)) ? moment(savings.start_date).format('D MMM, YYYY') : '- - -'}</Text>
                            </View>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.WITHDRAW_DATE}</Text>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value, SharedStyle.right]}>
                                    {isMoment(moment(savings.end_date)) ? moment(savings.end_date).format('D MMM, YYYY') : '- - -'}</Text>
                            </View>
                        </View>
                        <View style={styles.row}>
                            <View style={SharedStyle.halfColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.MATURITY_DATE}</Text>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value]}>
                                    {isMoment(moment(savings.end_date)) ? moment(savings.end_date).format('D MMM, YYYY') : '- - -'}</Text>
                            </View>
                            {!!savings.card && (
                                <View style={SharedStyle.halfColumn}>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.DEBIT_CARD}</Text>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value, SharedStyle.right]}>
                                        {`${savings.card.card_type.trim()||""} **** ${savings.card.card_last4||""}`}
                                    </Text>
                                </View>
                            )}
                             
                                <View style={SharedStyle.halfColumn}>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label, SharedStyle.right]}>{Dictionary.SAVINGS_MODE}</Text>
                                    <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.value, SharedStyle.right]}> {savings.isCollectionAuto? Dictionary.AUTOMATIC:Dictionary.MANUAL}</Text>
                                </View>
                            
                        </View>
                        <View style={styles.row}>
                            <View style={SharedStyle.fullColumn}>
                                <Text numberOfLines={1} style={[SharedStyle.normalText, SharedStyle.label]}>{Dictionary.STATUS_LABEL}</Text>
                                <Text numberOfLines={1}
                                    style={[SharedStyle.normalText, SharedStyle.value, styles.status, styles[`${status_code}Status`]]}>
                                    {status_name}
                                </Text>
                            </View>
                        </View>
                        {savings.data_sources?.pecunia && !savings.is_fixed && !savings.is_matured && !savings.archived && (
                            <TouchItem
                                style={styles.editButton}
                                onPress={() => {
                                    this.props.navigation.navigate('EditSavingsPlan', { savings });
                                    Util.logEventData('investment_edit', { savings_id: savings.id });
                                }}>
                                <Icon.Feather
                                    size={Mixins.scaleSize(18)}
                                    style={styles.editButtonIcon}
                                    name="edit" />
                                <Text style={[SharedStyle.normalText, styles.editButtonText]}>{Dictionary.EDIT_PLAN_BTN}</Text>
                            </TouchItem>
                        )}
                    </View>
                </ScrollView>
...
export default connect(mapStateToProps, mapDispatchToProps)(withNavigationFocus(SavingsDetail));