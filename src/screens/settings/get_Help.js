...
class GetHelpSupport extends Component {
  state = {
    support_phone: "07055554900",
    call_phone: "07000330330",
    support_mail: "customer@creditville.ng",
    support_website: "https://creditville.ng/",
  };
  componentDidMount() {
    BackHandler.addEventListener("hardwareBackPress", this.handleBackButton);

    const support_phone = remoteConfig()
      .getValue(`support_phone_${env().target}`)
      .asString();
    const support_mail = remoteConfig()
      .getValue(`support_mail_${env().target}`)
      .asString();
    const support_website = remoteConfig()
      .getValue(`support_website_${env().target}`)
      .asString();
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withNavigationFocus(GetHelpSupport));
