...
import { BackHandler, ActivityIndicator, StyleSheet, Text, View, ScrollView, Image } from 'react-native';
...
class PaymentMethods extends Component {
    ...
    handleSubmit = () => {
        ...
        let payment_method = selected_payment_method === 'card' ? { ...card } : selected_payment_method === 'account' ? { ...account } : {
        ...
        else this.props.navigation.navigate(redirect, { payment_data, payment_method });
    }
    ...
    render() {
        ...
        let accounts = this.state.enable_accounts ? this.props.payment.accounts : [];

        let accountSize = accounts.length;
        let accountCountText = Dictionary.ACCOUNT_COUNT;
        accountCountText = accountCountText.replace("%s", (accountSize + (this.state.enable_wallet ? 1 : 0)));

        const progress = this.props.navigation.getParam('progress');

        return (
            <View style={SharedStyle.mainContainer}>
                <MainHeader leftIcon="arrow-left" onPressLeftIcon={this.handleBackButton} title={Dictionary.PAYMENT_METHODS_HEADER} />
                    ...
                    {!this.props.loading && (
                        <View>
                            ...
                            {this.state.enable_cards && (
                                <View>
                                    <View style={styles.summary}>
                                        <Text style={[SharedStyle.normalText, styles.summaryText]}>{cardCountText}</Text>
                                        ...
                                        <ScrollView
                                            ...
                                            {this.props.payment.cards.map((card, index) => {
                                                ...
                                                return <TouchItem
                                                    ...
                                                    <LinearGradient
                                                        key={card.id}
                                                        colors={[isEven ? '#E561FA' : '#DFC905', isEven ? '#551CF7' : '#3AB449']}
                                                        start={{ x: 0.8, y: 2 }}
                                                        end={{ x: 1, y: 0.2 }}
                                                        style={[styles.card, styles.userCard]}>
                                                        <View style={styles.cardTop}>
                                                            <View>
                                                                {!(!!this.state.selected_payment_method && this.state.selected_payment_method === 'card' &&
                                                                    this.state.card.id === card.id) && (
                                                                        <Icon.FontAwesome name={'circle'} size={Mixins.scaleFont(32)} color={Colors.LIGHT_UNCHECKED_BG} />
                                                                    )}
                                                                {!!this.state.selected_payment_method && this.state.selected_payment_method === 'card' &&
                                                                    this.state.card.id === card.id && (
                                                                        <Icon.Ionicons name={'ios-checkmark-circle'} size={Mixins.scaleFont(32)} color={Colors.SUCCESS} />
                                                                    )}
                                                            </View>
                                                            <Image
                                                        ...
                                                        <View style={styles.cardBottom}>
                                                            <View style={styles.halfColumn}>
                                                                <Text numberOfLines={1} style={[SharedStyle.normalText, styles.label]}>{Dictionary.CARD_NUMBER_LABEL}</Text>
                                                                <Text numberOfLines={1} style={[SharedStyle.normalText, styles.value]}>
                                                                    <Text style={styles.mask}>XXXX</Text> <Text>{card.card_last4}</Text>
                                                                </Text>
                                                            </View>
                                                            <View style={styles.halfColumn}>
                                                                <Text numberOfLines={1} style={[SharedStyle.normalText, styles.label, styles.right]}>{Dictionary.CARD_EXP_LABEL}</Text>
                                                                <Text numberOfLines={1} style={[SharedStyle.normalText, styles.value, styles.right]}>{expiry}</Text>
                                                            </View>
                                                        </View>
                                                    </LinearGradient>
                                                </TouchItem>
                            ...
                            {(this.state.enable_accounts || this.state.enable_wallet) && (
                                <View>
                                    <View style={styles.summary}>
                                        <Text style={[SharedStyle.normalText, styles.summaryText]}>{accountCountText}</Text>
                                            ...
                                            <TouchItem
                                                ...
                                                <View style={styles.walletDetails}>
                                                    <View style={styles.walletAction}>
                                                        {!!this.state.selected_payment_method && this.state.selected_payment_method === 'wallet' && (
                                                            <Icon.Ionicons name={'ios-checkmark-circle'} size={Mixins.scaleFont(22)} color={Colors.SUCCESS} />
                                                        )}
                                                    </View>
                                                    <View>
                                                        <Text
                                                            style={[SharedStyle.normalText, styles.walletName]}
                                                            numberOfLines={1}>
                                                            {Dictionary.CREDITVILLE}
                                                        </Text>
                                                        <Text
                                                            style={[SharedStyle.normalText, styles.walletNumber]}
                                                            numberOfLines={1}>
                                                ...
                                                <Text
                                                    ...
                                                    numberOfLines={1}>
                                                    ₦{Util.formatAmount((this.props.user.user_data.deposit[this.props.user.user_data.deposit.length - 1]?.balance || this.props.wallet.wallet_balance) || 0)}
                                                </Text>
                                            </TouchItem>
                                        )}
                                        {accounts.map((account, index) => {
                                            return <TouchItem
                                                ...
                                                <View style={styles.accountAction}>
                                                    {!!this.state.selected_payment_method && this.state.selected_payment_method === 'account' &&
                                                ...
                                                <View style={styles.accountText}>
                                                    <Text
                                                        style={[SharedStyle.normalText, styles.accountName]}
                                                        numberOfLines={1}>
                                                        {account.account_name}
                                                    </Text>
                                                    <Text
                                                        style={[SharedStyle.normalText, styles.accountNumber]}
                                                        numberOfLines={1}>
...
export default connect(mapStateToProps, mapDispatchToProps)(withNavigationFocus(PaymentMethods));