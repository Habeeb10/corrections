...
export const getReferralActivities = (code) => {
    return dispatch => {
        dispatch({
            type: START_LOAD_REFERRAL_ACTIVITIES
        });
        Network.getUserReferralActivities(code)
            .then((result) => {
                dispatch({
                    referral_activities: result,
                    type: LOAD_REFERRAL_ACTIVITIES
                })
            }).catch((error) => {
                dispatch({
                    type: LOAD_REFERRAL_ACTIVITIES_FAILED,
                    referral_activities_error: error?.respData.message
                });
            });
    };
}

export const getReferralActivitiesRunning = (code) => {
    return dispatch => {
        dispatch({
            type: START_LOAD_REFERRAL_ACTIVITIES_RUNNING
        });
        Network.getReferralActivitiesRunning(code)
            .then((result) => {
                dispatch({
                    referral_activities: result,
                    type: LOAD_REFERRAL_ACTIVITIES_RUNNING
                })
            }).catch((error) => {
                dispatch({
                    type: LOAD_REFERRAL_ACTIVITIES_RUNNING_FAILED,
...
export const getReferralActivitiesMatured = (code) => {
    return dispatch => {
        dispatch({
            type: START_LOAD_REFERRAL_ACTIVITIES_MATURED
        });
        Network.getReferralActivitiesMatured(code)
            .then((result) => {
                dispatch({
                    referral_activities: result,
                    type: LOAD_REFERRAL_ACTIVITIES_MATURED
                })
            }).catch((error) => {
                dispatch({
                    type: LOAD_REFERRAL_ACTIVITIES_MATURED_FAILED,
                    referral_activities_error: error.message
                });
            });
    };
