import {
  START_LOAD_SAVINGS_PRODUCTS,
  LOAD_SAVINGS_PRODUCTS,
  LOAD_SAVINGS_PRODUCTS_FAILED,
  START_LOAD_SAVINGS_COLLECT_MODES,
  LOAD_SAVINGS_COLLECT_MODES,
  LOAD_SAVINGS_COLLECT_MODES_FAILED,
  START_LOAD_SAVINGS_FREQUENCIES,
  LOAD_SAVINGS_FREQUENCIES,
  LOAD_SAVINGS_FREQUENCIES_FAILED,
  START_LOAD_USER_SAVINGS,
  LOAD_USER_SAVINGS,
  LOAD_USER_SAVINGS_FAILED,
  UPDATE_APPLICATION,
  RESET_APPLICATION,
  UPDATE_SAVINGS_ARCHIVED_STATUS,
...
export default (state = initialState, action) => {
  switch (action.type) {
    ...
    case UPDATE_APPLICATION: {
      let savings_application = {
        ...state.savings_application,
    ...
    case LOAD_USER_SAVINGS: {
      ...
      user_savings_data.forEach((savings) => {
        let archived =
          savings.achieveStatus && savings.achieveStatus == "A" ? true : false;
        let status =
          savings.status === "ACTIVE"
            ? 1
            : savings.status === "IN_ARREARS"
            ? 2
            : 0;
        let savings_data = {
          id: savings.ID,
          ...
          interest_due_at_maturity: savings.interestAtMaturity || "0",
