...
class ArchiveSavingsWarning extends Component {
    render() {
        const { onAgree, onDisagree, isRestore } = this.props;
        return (
            <Modal
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? "position" : ""}>
                    <View style={[SharedStyle.modalContent, { height: Mixins.scaleSize(300) }]}>
                        <View style={SharedStyle.modalSlider} />
                        <View style={SharedStyle.modalPanel}>
                            <View style={styles.modalContent}>
                                <View style={styles.logoContainer}>
                                    <Icon.Entypo name={'archive'} size={Mixins.scaleFont(32)} color={Colors.CV_RED} />
                                </View>
                                <Text style={styles.title}>{isRestore ? Dictionary.RESTORE_SAVINGS_TITLE : Dictionary.ARCHIVE_SAVINGS_TITLE}</Text>
                                <Text style={styles.description}>{isRestore ? Dictionary.RESTORE_SAVINGS_DESSCRIPTION : Dictionary.ARCHIVE_SAVINGS_DESSCRIPTION}</Text>
                            </View>
...
export default ArchiveSavingsWarning;